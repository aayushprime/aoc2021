def part1(lines):
    ...
    for line in lines:
        ...
        if start[0] == end[0] or start[1] == end[1]:
            ...
            l = [int(start[i]), int(end[i])]
            l = [min(l), max(l)+1]
            for j in range(*l):
                key = start[0].zfill(3) + ':' + str(j).zfill(3) if i == 1 else str(j).zfill(3) + ':' + start[1].zfill(3)
                if key not in seen:
                    seen[key] = 1
                else:
                    seen[key] += 1
...
def part2(lines):
    ...
    for line in lines:
        ...
        if start[0] == end[0] or start[1] == end[1]:
            ...
            l = [min(l), max(l)+1]
            for j in range(*l):
                key = str(start[0]).zfill(3) + ':' + str(j).zfill(3) if i == 1 else str(j).zfill(3) + ':' + str(start[1]).zfill(3)
                if key not in seen:
                    seen[key] = 1
                else:
                    seen[key] += 1
